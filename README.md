...
**Status (May 2026):** the protocol surface, agent skill, and MCP server are wired, typed, and tested. Five of the nine tools run end-to-end: **`flyway_init`** produces a cryptographically signed identity (DID document, EdDSA-signed entity statement, ed25519 keypair); **`flyway_status`** reads it back and verifies signatures; **`flyway_recognize`** (+ `unrecognize`) writes signed peer-recognition entries; **`flyway_check`** reads and verifies incoming signals; and **`flyway_tension`** signs and delivers the first cross-murmuration signal envelope (S3 *Navigate via Tension*) into a peer's inbox via the local-fs transport ([ADR-0008](./docs/adr/0008-signal-transport-convention.md)). Signing is pluggable through the `Signer` interface ([ADR-0007](./docs/adr/0007-pluggable-signers-and-anchors.md)); a future `flyway-cardano` package will add a Cardano-resident signer and on-chain anchoring without touching `flyway-core`. The remaining four tools (`flyway_discover`, `flyway_propose`, `flyway_respond`, `flyway_exit`) still return "not yet implemented" — that's the next milestone.
...
| `flyway_respond`   | Respond to a proposal (accept/object/exit) or tension (acknowledge/dispute/dissolve/transfer) | Not yet wired  |
