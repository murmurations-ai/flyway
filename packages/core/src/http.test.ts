...
describe('assertPublicHttpsUrl — scheme + basic SSRF', () => {
  it('accepts a public https URL', () => {
    expect(() => {
      assertPublicHttpsUrl('https://directory.flyway.dev/x')
    }).not.toThrow()
    expect(() => {
      assertPublicHttpsUrl('https://[2606:4700::1]/x')
    }).not.toThrow() // public IPv6
  })
  ...
  it('refuses IPv4 loopback / private / link-local literals', () => {
    for (const h of [
      ...
      expect(() => {
        assertPublicHttpsUrl(`https://${h}/x`)
      }, h).toThrow(/private\/loopback/)
    }
  })
  ...
  it('allows a private host when allowPrivate is set', () => {
    expect(() => {
      assertPublicHttpsUrl('https://127.0.0.1/x', true)
    }).not.toThrow()
    expect(() => {
      assertPublicHttpsUrl('https://[::1]/x', true)
    }).not.toThrow()
  })
})
...
describe('fetchTextOverHttps — body', () => {
  ...
  it('enforces the byte cap at the boundary', async () => {
    // eslint-disable-next-line @typescript-eslint/require-await -- stub matches async fetch signature
    const under = (async () =>
      new Response('x'.repeat(100), { status: 200 })) as unknown as typeof fetch
    expect(
      await fetchTextOverHttps('https://x.dev/a', { fetchImpl: under, maxBytes: 100 }),
    ).toHaveLength(100)
    // eslint-disable-next-line @typescript-eslint/require-await -- stub matches async fetch signature
    const over = (async () =>
      new Response('x'.repeat(101), { status: 200 })) as unknown as typeof fetch
    await expect(
...
describe('fetchTextOverHttps — redirects (H-2)', () => {
  ...
  it('refuses a redirect to a private host WITHOUT contacting it', async () => {
    ...
    await expect(fetchTextOverHttps('https://a.dev/start', { fetchImpl })).rejects.toThrow(
      /private\/loopback/,
    )
  ...
  it('refuses a redirect that downgrades to http', async () => {
    ...
    const fetchImpl = (async () =>
      new Response(null, {
        status: 301,
        headers: { location: 'http://a.dev/insecure' },
      })) as unknown as typeof fetch
    await expect(fetchTextOverHttps('https://a.dev/start', { fetchImpl })).rejects.toThrow(
      /HTTPS-only/,
    )
  ...
  it('bounds the redirect chain', async () => {
    ...
    const fetchImpl = (async () => {
      ...
      return new Response(null, {
        status: 302,
        headers: { location: `https://a.dev/hop${String(n)}` },
      })
    }) as unknown as typeof fetch
    await expect(fetchTextOverHttps('https://a.dev/start', { fetchImpl })).rejects.toThrow(
      /too many redirects/,
